...
| **What a red check means** | Only that something is wrong: a silent failure, the dead-token signature, or a run that posted no review. A PR editing the review workflow makes the action refuse to run it (invariant 3's sibling: the file must match the default branch); that skip is reported **green** with a `::notice` and the same self-report comment, so the one check that proves a PR was reviewed never cries wolf. A skipped review still means this PR carries no review: the next PR after it merges gets one. |
