...
names), preserving everything else in the file — including any `ref` pin
(branch/tag) on the marketplace source (repo-owned; a marketplace source
honors `ref` but not `sha` — see `factory_stamp.py merge-settings`).
